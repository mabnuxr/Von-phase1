...
import { isApprovalTool, isGoogleCalendarApprovalTool } from '@vonlabs/design-components';
...
function detectApprovalFromArgs(
  ...
  if (parsed.operations && Array.isArray(parsed.operations) && parsed.summary) {
    ...
    const isCalendar = firstOp?.start_datetime || firstOp?.calendar_id || firstOp?.attendees_emails;
    ...
    if (isCalendar) {
      ...
      return {
        ...
        changes: isBulk
          ? ops.flatMap((op: { changes?: DetectedApprovalData['changes'] }) => op.changes || [])
          : firstOp?.changes,
  ...
  if (parsed.summary && parsed.action && (parsed.resource || parsed.target || parsed.record)) {
    return {
      toolCallId,
      summary: parsed.summary,
...
export function transformAguiToTimelineSteps(
  ...
  for (const wrapper of sortedEvents) {
    ...
    switch (event.type) {
      ...
      case 'STEP_STARTED': {
        ...
        const hasStepNumber = stepEvent.step_number !== undefined && stepEvent.step_number !== null;
        stepCounter++;
      ...
      case 'STEP_FINISHED': {
        ...
        const hasStepNumber =
          finishedEvent.step_number !== undefined && finishedEvent.step_number !== null;
        let step = hasStepNumber ? stepNumberMap.get(finishedEvent.step_number) : undefined;
        ...
        const matchesByNumber = hasStepNumber && currentStepNumber === finishedEvent.step_number;
        const matchesByName =
          finishedEvent.step_name && currentStepName === finishedEvent.step_name;
      ...
      case 'TEXT_MESSAGE_CONTENT': {
        ...
        if (
          ...
          (explicitFinalResponseMessageId && event.message_id === explicitFinalResponseMessageId)
        ) {
        ...
        currentTextStep.text = (currentTextStep.text || '') + (event.delta || '');
      ...
      case 'TOOL_CALL_START': {
        ...
        step.description = getToolDescription(toolName, toolArgsMap.get(toolId || ''));
      ...
      case 'TOOL_CALL_ARGS': {
        ...
        if (toolId) {
          ...
          if (!step) {
            ...
            step = stepNumber !== undefined ? stepNumberMap.get(stepNumber) : undefined;
          }
      ...
      case 'RUN_ERROR': {
        ...
        const hasPendingApproval = steps.some((s) => s.status === 'awaiting-approval');
  ...
  const isAwaitingApproval = steps.some((s) => s.status === 'awaiting-approval');
