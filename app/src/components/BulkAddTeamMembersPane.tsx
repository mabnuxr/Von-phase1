...
export function BulkAddTeamMembersPane() {
  const { addingBulkTeamMembers, setAddingBulkTeamMembers } =
    usePreferencesStore();
  const { user } = useUser();
  ...
  // Open a user-channel connection only while the pane is open. Cleans up
  // automatically on close because the pane unmounts.
  ...
  // Reset transient state when pane closes
  useEffect(() => {
    if (!addingBulkTeamMembers) {
      setFile(null);
      setJobId(null);
      setResponse(null);
  ...
  }, [addingBulkTeamMembers, reset]);

  const handleClose = () => {
    if (importMutation.isPending) return;
    setAddingBulkTeamMembers(false);
  };
  ...
  // Drag & drop handlers. preventDefault on dragover is required for drop to
  // fire; without it, the browser falls back to its default file-handling
  // (navigates to the dropped file) instead of giving us the event.
  ...
  // Pane-level guard so stray drops anywhere on the pane don't navigate away.
    ...
    <>
      ...
      <div
        className={`fixed top-0 right-0 h-full w-xl p-2 z-50 transform transition-transform duration-300 ease-in-out ${
          addingBulkTeamMembers ? "translate-x-0" : "translate-x-full"
        }`}
              ...
              <div className="space-y-4">
                ...
                <div className="h-2 w-full bg-gray-100 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      width:
                        total > 0 ? `${(completed / total) * 100}%` : "10%",
                    }}
                  />
                </div>
          ...
          <div className="px-6 py-4 border-t border-gray-200 shrink-0">
            <div className="flex items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-3">
                ...
                {isDone && (
                  <button
                    onClick={() => setAddingBulkTeamMembers(false)}
