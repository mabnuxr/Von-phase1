import usePreferencesStore, {
  DEFAULT_VONIQ_FIELDS,
} from "../store/preferencesStore";
import { VonIQFieldItem } from "./VonIQFieldItem";

export function VonIQFieldsPanel() {
  const {
    fieldsSearchTerm: searchTerm,
    setFieldsSearchTerm,
    voniqFieldCustomizations,
  } = usePreferencesStore();

  // Use local default fields instead of fetching from backend
  const defaultVonIQFields = DEFAULT_VONIQ_FIELDS;

  // Filter fields based on search term
  const filteredDefaultFields = defaultVonIQFields.filter(
    (field) =>
      field.sourceFieldDisplayName
        .toLowerCase()
        .includes(searchTerm.toLowerCase()) ||
      field.name.toLowerCase().includes(searchTerm.toLowerCase()),
  );

  // Helper to get enabled state for a field
  const isFieldEnabled = (fieldId: string): boolean => {
    const customization = voniqFieldCustomizations.find(
      (c) => c.fieldId === fieldId,
    );
    return customization ? customization.enabled : true; // Default to enabled
  };

  return (
    <div className="w-full h-full flex flex-col">
      {/* Fields List */}
      <div className="flex-1 overflow-y-auto px-6 pt-2 pb-6">
        <div className="space-y-6">
          {/* Header Row with Add New Button and Search */}
          <div className="flex flex-row items-center justify-between gap-4">
            {/* <button
              onClick={handleAddNewClick}
              className="px-4 py-2 text-sm font-medium text-white bg-gray-600 rounded-lg hover:bg-gray-700 transition-colors duration-200 hover:cursor-pointer"
            >
              + Add new
            </button> */}

            {/* Search Input */}
            <div className="relative flex-1 max-w-md">
              <input
                type="text"
                value={searchTerm}
                onChange={(e) => setFieldsSearchTerm(e.target.value)}
                placeholder="Search fields..."
                className="w-full px-4 py-2 pl-10 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-von-purple focus:border-transparent bg-white"
              />
              <svg
                className="absolute left-3 top-1/2 transform -translate-y-1/2 size-4 text-gray-400"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  strokeWidth={2}
                  d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"
                />
              </svg>
            </div>
          </div>

          {/* Default VonIQ Fields Section - SHOWN SECOND */}
          <div>
            <div className="mb-4">
              <h3 className="text-sm font-semibold text-gray-900 mb-1">
                Default Fields
              </h3>
              <p className="text-xs text-gray-600">
                These fields are automatically generated by VonIQ.
              </p>
            </div>
            {/* Vertical list layout */}
            <div className="space-y-3">
              {filteredDefaultFields.map((field) => (
                <VonIQFieldItem
                  key={field.id}
                  field={field}
                  enabled={isFieldEnabled(field.id)}
                  isUserDefined={false}
                />
              ))}
              {filteredDefaultFields.length === 0 && (
                <div className="text-sm text-gray-500 text-center py-8 border border-dashed border-gray-300 rounded-lg">
                  No default fields found
                  {searchTerm && ` matching "${searchTerm}"`}
                </div>
              )}
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}
