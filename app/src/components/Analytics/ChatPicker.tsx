...
import { ensureUTC, Tooltip, formatRelativeTime } from '@vonlabs/design-components';
...
export function ChatPicker({
  ...
  useEffect(() => {
    if (isRenaming) {
      ...
      const seed = activeTitle === 'Chat' || activeTitle === 'New chat' ? '' : activeTitle;
      setRenameValue(seed);
      setIsOpen(false);
        ...
        <div className="absolute top-full left-0 mt-1 bg-white border border-gray-100 rounded-2xl shadow-lg z-50 overflow-hidden max-h-80 overflow-y-auto p-1 min-w-[220px]">
          {isLoading ? (
            ...
            <div className="px-3 py-4 text-xs text-gray-400 text-center">
              {isDashboardMode ? 'No chats for this dashboard yet' : 'No chats yet'}
            </div>
          ) : isDashboardMode ? (
            <div>
