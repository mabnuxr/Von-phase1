...
const AnalyticsView: React.FC<AnalyticsViewProps> = ({
  ...
  //   0. dashboardVersion < 1 (never published) → "Publish dashboard"
  //      CTA. Fires regardless of the collab flag — a not-yet-published
  //      preview has nothing to draft and no other editors to lock out,
  //      so we skip the triad / Edit-mode dance and surface the single
  //      action that makes sense: publish it.
  ...
  const renderEditCluster = () => {
    if (dashboard.dashboardVersion < 1) {
      ...
      return (
        <Tooltip content="Publish this dashboard to your workspace. You'll be able to find it in the side panel once published.">
          <button
            type="button"
            onClick={!inFlight ? handleSaveFromEditMode : undefined}
            disabled={inFlight}
  ...
  return (
    <>
      ...
      >
        ...
        >
          {/* Save toast — pinned to the Canvas viewport top via a
              zero-height sticky wrapper so it stays visible even when
              the grid is scrolled. `h-0` keeps it from taking layout
              space; the absolute inner positions the pill 16px below
          ...
          {/* Refresh-in-progress banner — same sticky-wrapper trick so
              the indicator stays visible regardless of canvas scroll. */}
          <div className="sticky top-0 z-10 h-0">
            <AnimatePresence>
              {isRefreshing && (
                <motion.div
