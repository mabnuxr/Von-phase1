...
interface MoveToFolderModalProps {
  ...
  onConfirm: (config: { folderId: string; isNewFolder: boolean; newFolderName?: string }) => void;
  onCancel: () => void;
}
...
export function MoveToFolderModal({
  ...
  const handleConfirm = () => {
    ...
    if (isCreatingNew && !newFolderName.trim()) errs.name = "Folder name is required";
          ...
          >
            <div className="flex flex-col p-5">
              ...
              <p className="text-sm text-gray-600 mb-4">
                Adding <span className="font-medium text-gray-900">"{itemName}"</span> (chat) to a folder
              </p>
              ...
              {hasNoFolders ? (
                <div className="space-y-3">
                  <div className="py-4 text-center">
                    <FolderSimpleIcon size={32} weight="duotone" className="mx-auto text-gray-300 mb-2" />
                    <p className="text-sm text-gray-500">No folders available</p>
                    <p className="text-xs text-gray-400 mt-1">Create a new folder to organize your chats</p>
                  </div>
                  <button
                    onClick={() => { setSelectedId(CREATE_NEW); setIsCreatingNew(true); }}
