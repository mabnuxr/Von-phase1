...
import type { Message, TimelineStep, ToolCall, StepMessage } from '@vonlabs/design-components';
...
export const mockToolCalls: ToolCall[] = [
  {
    ...
    arguments: {
      query: 'SELECT account_name, SUM(amount) as revenue FROM opportunities GROUP BY account_name ORDER BY revenue DESC LIMIT 5',
    },
    status: 'success',
...
export const mockTimelineSteps: TimelineStep[] = [
  {
    ...
    status: 'complete',
    type: 'reasoning',
    description: 'Parsing the user query to identify the key metrics and time period requested.',
  },
  {
    id: 'step-2',
    ...
    type: 'tool_call',
    source: 'salesforce',
    description: 'Executing SOQL query to retrieve closed-won opportunities from Q4.',
    artifact: {
  ...
  },
  {
    ...
    source: 'voniq',
    category: 'sql',
    description: 'Running SQL aggregation to calculate total revenue per account.',
    artifact: {
...
df_sorted = df.sort_values('q4_revenue', ascending=False).head(5)`,
    description: 'Computing quarter-over-quarter growth rates for each account.',
  },
  {
    id: 'step-5',
    ...
    status: 'in-progress',
    type: 'output',
    description: 'Formatting the analysis results into a comprehensive response.',
  },
];
...
- **Potential revenue at risk**: $2.1M`,
    dataSources: [
      { name: 'Salesforce Opportunities', record_count: 1250, description: 'Closed lost and churned accounts' },
      { name: 'Support Tickets', record_count: 3400, description: 'Customer support interactions' },
      { name: 'Usage Analytics', record_count: 8900, description: 'Product usage data' },
    ],
  },
};
...
export const mockArtifactContent: ArtifactResponse = {
  ...
  content: {
    ...
    query: {
      ...
      dialect: 'postgresql',
      statement: 'SELECT account_name, SUM(amount) as revenue, ROUND((SUM(amount) - LAG(SUM(amount)) OVER (ORDER BY account_name)) / LAG(SUM(amount)) OVER (ORDER BY account_name) * 100, 0) as growth FROM opportunities WHERE close_date >= \'2024-10-01\' AND stage = \'Closed Won\' GROUP BY account_name ORDER BY revenue DESC LIMIT 5',
    },
  },
};
...
export const mockFailedMessage: Message = {
  ...
  status: 'failed',
  errorMessage: 'Failed to connect to Salesforce. Please check your connection and try again.',
  isStreaming: false,
};
...
export function createMockStreamingMessage(options: {
  ...
  const { content = '', progress = 0, hasToolCalls = false, isV2 = false } = options;

  if (isV2) {
    const steps = mockTimelineSteps.map((step, index) => ({
      ...step,
      status: index < Math.floor(progress / 20) ? 'complete' as const :
              index === Math.floor(progress / 20) ? 'in-progress' as const : 'pending' as const,
    }));
